 ...
 * @DataMongoTest
 *
 * @RunWith(SpringRunner.class) public class TailableOrderQueryTest {
 *
 * @Autowired private ReactiveMongoTemplate operations;
 *
 ...
 * .collectionExists(Order.class) .flatMap(exists -> exists ?
 * operations.dropCollection(Order.class) : Mono.just(exists))
 ...
 *
 * @Test public void tail() { Queue<Order> people = new ConcurrentLinkedQueue<>();
 * Mono<Order> then = this.writeAndWait().then(this.writeAndWait());
 * StepVerifier.create(then).expectNextCount(1).verifyComplete();
 * this.repository.findByProductId("1") // .doOnNext(people::add) // .doOnComplete(() ->
 * log.info("complete")) // .doOnTerminate(() -> log.info("terminated")) // .subscribe();
 ...
 *
 * private Mono<Order> writeAndWait() { return repository.save(new
 * Order(UUID.randomUUID().toString(), "1")); }
 *
 * }
 */