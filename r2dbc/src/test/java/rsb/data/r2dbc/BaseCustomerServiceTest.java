...
public abstract class BaseCustomerServiceTest {
	...
	public void executeRollback() {
		var customers = this.operator
				.execute(status -> core(getService().saveAllEmailsExecute(emails)));
	...
	public void transactionalRollback() {
		var customers = core(this.getService().saveAllEmailsTransactional(this.emails));
		this.runTransactionalTest(customers);
	}

	Flux<Customer> core(Flux<Customer> queryResults) {
		Flux<Customer> customerFlux = this.getRepository() //
				.findAll()//
				.flatMap(c -> this.getRepository().deleteById(c.getId())) //
				.thenMany(queryResults) //
